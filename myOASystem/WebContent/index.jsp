		...
		<div class="main-wapper">
			...
			<div id="siderbar" class="siderbar-wapper">
				...
				<div class="panel-group menu" id="accordion" role="tablist"
					...
					<c:forEach var="menu" items="${activeUser.menuTree}"
						varStatus="state">
						<div class="panel panel-inner">
							<div class="panel-heading panel-heading-self" role="tab"
								id="heading${state.index}">
								<h4 class="panel-title">
									<a data-toggle="collapse" data-parent="#accordion"
										href="#collapse${state.index}"
										<c:if test="${state.index==0}">aria-expanded="true"</c:if>
										<c:if test="${state.index>0}">aria-expanded="false"</c:if>
										aria-controls="collapse${state.index}"> <c:if
											test="${menu.name=='报销管理' }">
											<span class="glyphicon glyphicon-pencil"></span> ${menu.name}</c:if>
										<c:if test="${menu.name=='请假管理' }">
											<span class="glyphicon glyphicon-list-alt"></span> ${menu.name}</c:if>
										<c:if test="${menu.name=='流程管理' }">
											<span class="glyphicon glyphicon-th-large"></span> ${menu.name}</c:if>
										<c:if test="${menu.name=='系统管理' }">
											<span class="glyphicon glyphicon-list"></span> ${menu.name}</c:if>
									</a>
								</h4>
							</div>
							<div id="collapse${state.index}"
